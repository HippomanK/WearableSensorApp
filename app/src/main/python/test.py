...
class SpecData(Dataset):
    ...
    """
    def __init__(self, input_data, n_fft=48):
...
def evaluate(dataset):
    # read csv file
    filename = os.path.join(dirname(__file__), 'test_data(old)', 'walking', 'Sheet1.csv')
    model_path = os.path.join(dirname(__file__), 'test_data(old)', 'best.pth')
